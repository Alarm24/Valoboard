...
public class BrimstoneUltimateHandler implements EventHandler<ActionEvent> {
    private final Agent agent;
    private final BoardPane boardPane;
    private final AgentManager agentManager;


    public BrimstoneUltimateHandler(Agent agent) {
        this.agent = agent;
    ...
    public void handle(ActionEvent mouseEvent) {
        ...
        List<Coordinate> availableUlti = agent.getAvailableSkillRange(agent.getShootingDistance());
        for (Coordinate destination : availableUlti) {
            Label target = boardPane.getOneCell(destination);
            target.setStyle(BoardPane.defaultAvailableShootStyle);
            target.setOnMouseClicked(new AoeDamageHandler(agent, destination));
        }
    }
